...
public class XPTreasureChest
  extends TreasureChest
{
  private Location location;
  private boolean isCancelled = false;
  
  public XPTreasureChest()
  {
    registerEvents();
    summonHologram();
  }
  ...
  public void openChest(final Player p)
  {
    setInUse(true);
    changeChestState();
    int amount = new Random().nextInt(4500) + 7500;
    DBPlayer.loadPlayer(p).setExpScaled(DBPlayer.loadPlayer(p).getExpScaled() + amount);
    p.sendMessage("§d+" + amount + " Experience");
    final XPTreasureChest inst = this;
    new BukkitRunnable()
    {
      public void run()
      {
    ...
    for (int i = 0; i < 30; i++)
    {
      ...
      new BukkitRunnable()
      {
        public void run()
        {
          ItemStack i2 = ItemUtils.setName(new ItemStack(Material.EXP_BOTTLE), i3);
          final Item i4 = p.getWorld().dropItem(XPTreasureChest.this.getLocation().clone().add(0.5D, 1.0D, 0.5D), i2);
          ...
          Player p2;
          for (Iterator localIterator = p.getWorld().getPlayers().iterator(); localIterator.hasNext(); p2.playSound(XPTreasureChest.this.getLocation(), Sound.ITEM_PICKUP, 1.0F, 0.0F)) {
  ...
  public Location getLocation()
  {
    ...
    try
    {
      return LocationManager.getLocation(((World)Bukkit.getWorlds().get(0)).getName(), "xpchest");
    }
